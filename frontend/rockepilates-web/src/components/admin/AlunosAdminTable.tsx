...
export function AlunosAdminTable({ alunosIniciais }: AlunosAdminTableProps) {
    ...
    const [assinaturaProcessando, setAssinaturaProcessando] = useState<number | null>(null);
    const [erro, setErro] = useState<string | null>(null);

    const [modalAberto, setModalAberto] = useState(false);
    const [alunoSelecionado, setAlunoSelecionado] = useState<AlunoAdmin | null>(null);
    ...
    async function handleAbrirHistorico(aluno: AlunoAdmin) {
        try {
            ...
            setErroHistorico("Não foi possível carregar o histórico de pagamentos.");
        } finally {
                                ...
                                >
                                    ...
                                    <td className="px-5 py-5 text-right">
                                        <div className="flex justify-end gap-2">
                                            ...
                                            {podeMarcarComoPago(aluno.status) ? (
                                                <button
                                                    type="button"
                                                    onClick={() =>
